...
public class MacroController : IPrintService, IDisposable // 实现 IDisposable
{
    ...
    /// 创建播放器策略
    /// </summary>
    private static Dictionary<Type, IEventPlayer> CreatePlayers(ISimulator simulator)
    {
        var players = new Dictionary<Type, IEventPlayer>
        {
            ...
            { typeof(MouseEvent), new SimulatorMouseEventPlayer(simulator) },
            { typeof(KeyboardEvent), new SimulatorKeyboardEventPlayer(simulator) },
        };
